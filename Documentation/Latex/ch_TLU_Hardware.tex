...
The \gls{dut}s are connected to the \gls{tlu} using standard size \gls{hdmi} connectors\footnote{The miniTLU hardware used mini\gls{hdmi} connectors.}.\\
...
\end{alertinfo}
Table~\ref{tab:HDMIpins} shows the pin naming and the corresponding output enable signal. Figure~\ref{fig:HDMI_Pinout} shows the pin-numbering for a HDMI plug. The clock pairs have two different enable signals to select the clock source (see section~\ref{ch:clock} for more details). In general only one of the clock sources should be active at any time.\\
...
\begin{figure}
  \centering
  \includegraphics[width=.80\textwidth]{./Images/hdmi-pins.png}
  \caption{Pin numbering for an HDMI plug.}\label{fig:HDMI_Pinout}
\end{figure}\\
